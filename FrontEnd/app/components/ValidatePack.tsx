...
const ValidatePack = () => {
  ...
  const handleValidate = async () => {
    ...
    if (!pickupCode.trim()) {
      ...
      setErrorDialogText(
        t('packs.validation_enter_code', {
          defaultValue: 'Introduza o código de levantamento.',
        }),
      );
    ...
    setLoading(true);
    try {
      const response = await apiFetch(`/packs/validar`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setResultData({
          success: true,
          title: t('packs.validation_success_title', {
            defaultValue: 'Pacote Validado',
          }),
          message: t('packs.validation_deliver_prize', {
            defaultValue: 'Entregar o prémio ao cidadão.',
          }),
        ...
        setResultData({
          success: false,
          title: t('packs.validation_error_title', {
            defaultValue: 'Código Inválido',
          }),
          message:
            data.message ||
            t('packs.validation_error_default', {
              defaultValue: 'Não foi possível validar este código.',
            }),
        });
        setResultDialogVisible(true);
      }
  ...
  return (
    <>
      ...
      <Surface style={{ flex: 1, backgroundColor: theme.colors.background }}>
        <SafeAreaView style={{ flex: 1 }} edges={['left', 'right']}>
          ...
          >
            ...
            >
              {t('packs.validate_subtitle', {
                defaultValue: 'Levantamento de Prémios',
              })}
            </Text>
            <Text
              variant="bodyMedium"
            ...
            >
              {t('packs.validate_instructions', {
                defaultValue:
                  'Peça ao cidadão o código de levantamento que recebeu na aplicação quando comprou o pacote. Introduza o código abaixo para validar e entregar o prémio.',
              })}
            </Text>
            ...
            <View style={{ marginBottom: 24 }}>
              <CustomTextInput
                label={t('packs.pickup_code_label', {
                  defaultValue: 'Código de levantamento',
                })}
                value={pickupCode}
                onChangeText={setPickupCode}
                placeholder="TD-2026-XXXXXXXX"
                accessibilityLabel={t('accessibility.pickup_code_input', {
                  defaultValue:
                    'Campo para introduzir o código de levantamento',
                })}
                accessibilityHint={t('accessibility.pickup_code_hint', {
                  defaultValue:
                    'Introduza o código que o cidadão recebeu ao comprar o pacote',
                })}
              />
              <Text
                variant="labelSmall"
              ...
              >
                {t('packs.code_format_hint', {
                  defaultValue:
                    'Formato: TD-AAAA-XXXXXXXX (ex.: TD-2026-A3F8K2)',
                })}
              </Text>
            </View>
            ...
            <CustomButton
              ...
              className="w-full"
              accessibilityLabel={t('accessibility.validate_pack_btn', {
                defaultValue: 'Validar código e entregar prémio',
              })}
              accessibilityHint={t('accessibility.validate_pack_hint', {
                defaultValue:
                  'Clica para verificar o código e marcar o pacote como entregue',
              })}
            >
              {loading
                ? t('common.processing', { defaultValue: 'A processar...' })
                : t('packs.validate_button', {
                    defaultValue: 'Validar Código',
                  })}
            </CustomButton>
          </ScrollView>
        </SafeAreaView>
      ...
      <CustomDialog
        ...
        buttonColor={
          resultData?.success ? theme.colors.primary : theme.colors.error
        }
        textColor={
          resultData?.success ? theme.colors.onPrimary : theme.colors.onError
        }
            ...
            {resultData.success ? (
              <View style={{ gap: 8 }}>
                ...
                >
                  ...
                  >
                    {t('packs.prize_to_deliver', {
                      defaultValue: 'Prémio a entregar',
                    })}
                  </Text>
                  <Text
                    variant="titleMedium"
                ...
                {resultData.citizenName ? (
                  ...
                  >
                    <Text style={{ fontWeight: 'bold' }}>
                      {t('packs.citizen', { defaultValue: 'Cidadão' })}:
                    </Text>{' '}
                    {resultData.citizenName}
                  </Text>
                ) : null}
