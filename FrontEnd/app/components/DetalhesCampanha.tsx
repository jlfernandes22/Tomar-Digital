...
import { Surface, Text, IconButton, Divider, useTheme } from "react-native-paper";
...
const DetalhesCampanha = ({ visible, campaign, onClose }: DetalhesProps) => {
  ...
  try {
    const response = await fetch(`${API_URL}/campanhas/aderir`, {
      ...
      body: JSON.stringify({ businessId: negocioSelecionado, campaignId: campaign._id })
    });
...
useEffect(() => {
  const carregarNegocios = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/negociosCae?cae=${caeParaBuscar}`, {
      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', padding: 20 }}>
        <Surface style={{ width: '100%', maxHeight: '80%', borderRadius: 24, padding: 20 }}>
          ...
          <ScrollView style={{ flexGrow: 0 }}>
            {passo === 1 ? (
              <View>
                <Text variant="titleMedium" style={{ marginBottom: 10 }}>1. Selecione o negócio:</Text>
                ...
                <CustomButton 
                  style={{ backgroundColor: theme.colors.onBackground, marginTop: 10 }}
                  onPress={() => negocioSelecionado ? setPasso(2) : alert("Selecione um negócio!")}
                >
                  Continuar
                </CustomButton>
              </View>
              ...
              <View style={{ alignItems: 'center', padding: 20 }}>
                <Text style={{ textAlign: 'center', marginBottom: 20 }}>Confirma a adesão à campanha "{campaign.titulo}"?</Text>
                <View style={{ flexDirection: 'row', gap: 10 }}>
                  <CustomButton onPress={() => setPasso(1)}>Voltar</CustomButton>
                  <CustomButton 
