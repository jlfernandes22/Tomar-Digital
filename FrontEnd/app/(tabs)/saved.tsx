...
import { Surface, useTheme, Text, TouchableRipple, ActivityIndicator, Button } from "react-native-paper"; 
  ...
  return (
    <Surface style={{ flex: 1 }}>
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View className="p-6">
          <Text variant="headlineMedium" className="text-center font-bold text-[#FF6600]">
            Os Meus Favoritos
          </Text>
        </View>

        {loading ? (
          <ActivityIndicator animating={true} size="large" color="#FF6600" className="mt-20" />
        ) : (
          <FlatList
            data={favoritos}
            ...
            ListEmptyComponent={(
              <View className="flex-1 items-center justify-center px-10 pb-20">
                <Image 
                  ...
                  style={theme.dark ? { tintColor: 'white' } : { tintColor: 'black'}} 
                  resizeMode="contain"
                  importantForAccessibility="no-hide-descendants"
                  accessibilityElementsHidden={true}
                />

                <Text variant="titleLarge" className="font-bold text-center mb-2">
                  Lista vazia
                </Text>
                <Text variant="bodyMedium" className="text-center opacity-70 mb-8">
